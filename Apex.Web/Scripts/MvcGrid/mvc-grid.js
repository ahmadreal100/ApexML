...
var MvcGridFilter = (function () {
    ...
    MvcGridFilter.prototype = {
        render: function (grid, popup, filter) {
            ...
            /*AhmadR New Code.*/
            if (filter.name === "date") {
                popup.find('.first-filter .mvc-grid-value').val(jalaliConvertorString(filter.first.value, true));
                popup.find('.second-filter .mvc-grid-value').val(jalaliConvertorString(filter.second.value, true));
            } else {
        ...
        apply: function (grid, column, popup) {
            ...
            if (grid.filterMode != 'FilterRow' && column.filter.hasOptions) {
                ...
                /*AhmadR New Code.*/
                if (column.filter.name === "date") {
                    column.filter.first.value = jalaliConvertorString(container.find('.first-filter .mvc-grid-value').val());
                    column.filter.second.value = jalaliConvertorString(container.find('.second-filter .mvc-grid-value').val());
                } else {
...
var MvcGridDateFilter = (function (base) {
    ...
    MvcGridDateFilter.prototype.bindValue = function (grid, column, popup) {
        if ($.fn.datepicker) {
            popup.find('.mvc-grid-value').each(function (i) {
                $(this).attr("id", "mvcgrid-datepicker-" + (i + 1));
                $("#mvcgrid-datepicker-" + (i + 1)).buildKamaDatepicker();
            })
        }
